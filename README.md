...
> * **Decomposition Required:** For standard L1/L2 threat hunting and alert engineering, these logical structures should be **decomposed**—stripping away complex engineering components to focus on a "Single Baseline Truth" with bounded context. "Splitting" the rule does not mean losing the big picture. We use Correlated Incidents in the SIEM to stitch these separate alerts together into a single Attack Story. The Detection Rule is the sensor; the Incident is the narrative.
>
